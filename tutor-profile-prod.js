document.addEventListener("DOMContentLoaded", function () {
  const STRIPE_WORKER_URL = "https://tc-production-stripe.tutorchooser.workers.dev/create-checkout-session";
  const FILE_UPLOAD_WORKER_URL = "https://tc-production-file-upload.tutorchooser.workers.dev/";
  ...
  function showUIMessage(message, type = 'error', duration = 5000) {
    ...
    try {
      ...
      const res = await fetch("https://production.tutorchooser.workers.dev", {
  ...
  async function getCompletionPercentage(itemId) {
    try {
      const token = await window.$memberstackDom.getMemberCookie();
      const res = await fetch(`https://tc-production-profile.tutorchooser.workers.dev/completion/${itemId}`, {
  ...
  async function updateColumnsAndGetCompletion(itemId, columnUpdates) {
    ...
    try {
      const token = await window.$memberstackDom.getMemberCookie();
      const res = await fetch("https://tc-production-profile.tutorchooser.workers.dev/completion", {
        method: "POST", headers: { "Content-Type": "application/json", "Authorization": token },
        body: JSON.stringify(payload)
      });
...
async function createCheckoutSession(paymentType, button) {
    ...
    try {
      ...
      const res = await fetch("https://tc-production-stripe.tutorchooser.workers.dev/create-checkout-session", {
        method: "POST",
        headers: {
